...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // 用户管理API - 与前端保持一致（前端调用/api/admin/users）
      router.get('/api/admin/users', (req, env, ctx) => handleAdmin.getUsers(req, env, ctx));
      router.post('/api/admin/users', (req, env, ctx) => handleAdmin.createUser(req, env, ctx));
      router.put('/api/admin/users/:id', (req, env, ctx) => handleAdmin.updateUser(req, env, ctx));
      router.delete('/api/admin/users/:id', (req, env, ctx) => handleAdmin.deleteUser(req, env, ctx));
      router.put('/api/admin/users/:id/password', (req, env, ctx) => handleAdmin.changeUserPassword(req, env, ctx));
      router.put('/api/admin/users/:id/status', (req, env, ctx) => handleAdmin.toggleUserStatus(req, env, ctx));
