...
echo -e "${YELLOW}1. 修复ProcessManager.js中的FFmpeg参数...${NC}"

# 备份当前文件
cp src/services/ProcessManager.js src/services/ProcessManager.js.error-backup-$(date +%Y%m%d_%H%M%S)

# 创建修复后的ProcessManager.js
...
class ProcessManager {
  constructor() {
    ...
    // 修复的超时设置
    this.startupTimeout = 25000; // 启动超时：25秒
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        ffmpegProcess.stderr.on('data', (data) => {
          ...
          // 检查参数错误
          if (error.includes('out of range') || 
              error.includes('incorrect codec parameters') ||
              error.includes('Numerical result out of range')) {
            if (!resolved) {
              resolved = true;
              clearTimeout(startupTimer);
              logger.error(`Stream ${streamId} parameter error:`, error);
              this.stopStream(streamId);
              reject(new Error(`Parameter error: ${error}`));
            }
          }
...
echo -e "${GREEN}✅ FFmpeg参数修复完成${NC}"

echo -e "${YELLOW}2. 重启服务应用修复...${NC}"
pm2 restart vps-transcoder-api

# 等待服务重启
sleep 5

echo -e "${YELLOW}3. 测试修复后的转码功能...${NC}"

# 清理之前的测试文件
rm -rf /var/www/hls/test_* 2>/dev/null || true

# 测试转码API
echo -e "${BLUE}测试修复后的转码功能...${NC}"
TEST_STREAM_ID="test_fixed_$(date +%H%M%S)"
...
if echo "$RESPONSE" | grep -q "success.*true"; then
    ...
    for i in {1..45}; do
        if [ -f "/var/www/hls/$TEST_STREAM_ID/playlist.m3u8" ]; then
            ...
            # 测试外部访问
            echo -e "${BLUE}测试外部HLS访问...${NC}"
            EXTERNAL_TEST=$(curl -s -I "https://yoyo-vps.5202021.xyz/hls/$TEST_STREAM_ID/playlist.m3u8" | head -1)
            if echo "$EXTERNAL_TEST" | grep -q "200 OK"; then
                echo -e "${GREEN}✅ 外部HLS访问正常!${NC}"
                echo -e "${CYAN}外部访问地址: https://yoyo-vps.5202021.xyz/hls/$TEST_STREAM_ID/playlist.m3u8${NC}"
            else
                echo -e "${RED}❌ 外部HLS访问失败: $EXTERNAL_TEST${NC}"
            fi
            
            break
        else
            echo -e "${YELLOW}等待中... ($i/45)${NC}"
            sleep 2
        fi
...
echo -e "${GREEN}🎉 FFmpeg参数修复完成!${NC}"
echo -e "${BLUE}==================== 修复信息 ====================${NC}"
echo -e "${YELLOW}修复内容:${NC} 移除超出范围的timeout参数"
echo -e "${YELLOW}启动超时:${NC} 25秒"
echo -e "${YELLOW}连接参数:${NC} 简化的重连机制"
