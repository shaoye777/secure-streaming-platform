...
ssh root@<VPS_IP> "cd /tmp/github/secure-streaming-platform/vps-transcoder-api && chmod +x vps-simple-deploy.sh && ./vps-simple-deploy.sh"
...
- **隧道域名**: `https://tunnel-*.yoyo-vps.your-domain.com` (Cloudflare Tunnel优化)
- **服务器IP**: `<VPS_IP>`
- **SSH端口**: `22` (标准SSH连接端口)
...
#### VPS连接信息
- **SSH连接**: `ssh root@<VPS_IP>`
- **SCP文件传输**: `scp file.js root@<VPS_IP>:/path/to/destination/`
- **API访问**: `https://yoyo-vps.your-domain.com` (通过Nginx代理到端口52535)
...
DIRECT_ENDPOINTS: {
  API: 'https://yoyo-vps.your-domain.com',    // ← 默认443端口
  HLS: 'https://yoyo-vps.your-domain.com',    // ← 默认443端口
  HEALTH: 'https://yoyo-vps.your-domain.com'  // ← 默认443端口
}
```
...
2. **导航到规则配置**：选择域名 `your-domain.com` → 规则 → Origin Rules
3. **创建规则**（如果不存在）：
...
**规则效果**：
- 所有访问 `yoyo-vps.your-domain.com` 的请求（无论来自浏览器还是Workers）
...
┌────────────────────────────────────────────────────────────────┐
│ Workers代码配置                                                  │
│ URL: https://yoyo-vps.your-domain.com/hls/xxx/playlist.m3u8       │
│                                    ↓ (默认443端口)              │
└────────────────────────────────────────────────────────────────┘
                        ↓
┌────────────────────────────────────────────────────────────────┐
│ Workers内部fetch请求                                             │
│ fetch('https://yoyo-vps.your-domain.com/hls/...')                 │
│                                    ↓                            │
└────────────────────────────────────────────────────────────────┘
                        ↓
┌────────────────────────────────────────────────────────────────┐
│ Cloudflare边缘节点 (Edge)                                        │
│ 1. 接收请求: yoyo-vps.your-domain.com:443                          │
│ 2. 匹配Origin Rules规则8                                        │
│ 3. 【自动端口改写】443 → 52535                                  │
│ 4. 解析DNS: yoyo-vps.your-domain.com → <VPS_IP>            │
│                                    ↓                            │
└────────────────────────────────────────────────────────────────┘
                        ↓
┌────────────────────────────────────────────────────────────────┐
...
const hlsUrl = 'https://yoyo-vps.your-domain.com/hls/xxx/playlist.m3u8';
const response = await fetch(hlsUrl);

// ❌ 错误：手动指定端口号（不需要）
const hlsUrl = 'https://yoyo-vps.your-domain.com:52535/hls/xxx/playlist.m3u8';
```
...
server {
    ...
    server_name yoyo-vps.your-domain.com;
...
curl https://yoyo-vps.your-domain.com/health
# 预期: 200 OK (通过443访问，Cloudflare转发到52535)

# 3. 测试HLS文件访问
curl https://yoyo-vps.your-domain.com/hls/stream_xxx/playlist.m3u8
# 预期: 返回m3u8文件内容

# 4. 检查响应头（验证是Nginx返回）
curl -I https://yoyo-vps.your-domain.com/health
...
**隧道模式（使用cloudflared）**：
```
Workers → tunnel-hls.yoyo-vps.your-domain.com → cloudflared → localhost:52535
特点: 独立的隧道服务，中国用户优化
...
Response: {
  ...
  "data": {
    "channelId": "stream_ensxma2g",
    "channelName": "二楼教室1",
    "hlsUrl": "https://yoyo-vps.your-domain.com/hls/stream_ensxma2g/playlist.m3u8",
...
graph TD
    ...
    N --> O{URL包装}
    O -->|直连模式| P[yoyoapi.your-domain.com/hls/...]
    O -->|代理模式| Q[yoyoapi.your-domain.com/tunnel-proxy/hls/...]
    O -->|隧道模式| R[tunnel-hls.yoyo-vps.your-domain.com/hls/...]
...
#### 基础规范
1. **注意特定命令带上退出点**: 使用 `pm2 logs vps-transcoder-api --lines XX` 这种命令，要带上自动退出点，否则会导致持续等待，卡死对话。例如可以使用 `ssh root@<VPS_IP> "pm2 logs vps-transcoder-api --lines 20 --nostream`
...
```bash
# 1. 删除损坏的仓库
ssh root@<VPS_IP> "rm -rf /tmp/github/secure-streaming-platform"

# 2. 重新克隆
ssh root@<VPS_IP> "mkdir -p /tmp/github && cd /tmp/github && git clone git@github.com:shao-ye/secure-streaming-platform.git"
...
# 检查API认证
curl -H 'X-API-Key: <API_SECRET_KEY>' https://yoyo-vps.your-domain.com/api/simple-stream/health
```
...
```powershell
# 1. 检查VPS状态
Invoke-RestMethod -Uri "https://yoyo-vps.your-domain.com/api/deployment/status" -Method GET -TimeoutSec 10

# 2. 执行一键部署  
$deployBody = @{ scriptName = "integrate-proxy-streaming.sh" } | ConvertTo-Json
Invoke-RestMethod -Uri "https://yoyo-vps.your-domain.com/api/deployment/deploy/complete" -Method POST -Body $deployBody -ContentType "application/json" -TimeoutSec 300

# 3. 验证部署结果
Invoke-RestMethod -Uri "https://yoyo-vps.your-domain.com/api/proxy/status" -Method GET -TimeoutSec 10
```
...
1. 本地修改代码
2. 测试验证功能
3. 上传到VPS: scp local_file root@<VPS_IP>:/opt/yoyo-transcoder/
4. 重启服务: ssh root@<VPS_IP> "pm2 restart vps-transcoder-api"
5. 验证部署结果
```
...
5. 在 https://yoyo.your-domain.com 验证功能
```
...
**发现的问题**:
- 隧道模式开启后，浏览器直接访问 `tunnel-hls.yoyo-vps.your-domain.com` 触发SSL握手失败
...
```
旧架构（SSL问题）:
浏览器 → tunnel-hls.yoyo-vps.your-domain.com ❌ SSL握手失败
...
router.get('/tunnel-proxy/hls/:streamId/:file', async (req, env, ctx) => {  const { streamId, file } = req.params;
  ...
  try {
    ...
    // 故障转移到直连端点
    const directUrl = `https://yoyo-vps.your-domain.com/hls/${streamId}/${file}${queryString}`;
...
- **错误配置**: `tunnel-hls.yoyo-vps.your-domain.com` → `http://localhost:8080` (File Browser)
- **正确配置**: `tunnel-hls.yoyo-vps.your-domain.com` → `http://localhost:52535` (Nginx HLS服务)
...
```bash
# 隧道端点测试结果 (修复后)
✅ tunnel-health.yoyo-vps.your-domain.com/health - 连接成功
✅ tunnel-api.yoyo-vps.your-domain.com/health - 连接成功  
✅ tunnel-hls.yoyo-vps.your-domain.com/hls/stream_xxx/playlist.m3u8 - HLS服务正常
...
# DNS解析状态
tunnel-api.yoyo-vps.your-domain.com → CNAME → yoyo-streaming.cfargotunnel.com
tunnel-hls.yoyo-vps.your-domain.com → CNAME → yoyo-streaming.cfargotunnel.com  
tunnel-health.yoyo-vps.your-domain.com → CNAME → yoyo-streaming.cfargotunnel.com
```
...
```
https://yoyoapi.your-domain.com/api/login              # 用户登录
https://yoyoapi.your-domain.com/api/logout             # 用户登出
https://yoyoapi.your-domain.com/api/me                 # 获取当前用户信息
https://yoyoapi.your-domain.com/api/streams            # 获取流列表
https://yoyoapi.your-domain.com/api/play/:id           # 启动转码流
https://yoyoapi.your-domain.com/api/stop/:id           # 停止转码流
https://yoyoapi.your-domain.com/api/stream/:id/status  # 查询流状态
https://yoyoapi.your-domain.com/hls/:streamId/:file    # HLS文件代理
```
...
#### 传统API端点（向后兼容）
```
https://yoyo-vps.your-domain.com/health                # 健康检查
https://yoyo-vps.your-domain.com/api/start-stream      # 启动转码
https://yoyo-vps.your-domain.com/api/stop-stream       # 停止转码
https://yoyo-vps.your-domain.com/api/streams           # 获取运行中的流
https://yoyo-vps.your-domain.com/api/stream/:streamId  # 获取流信息
https://yoyo-vps.your-domain.com/hls/                  # HLS文件服务
```

#### 新简化架构API端点（推荐使用）
```
https://yoyo-vps.your-domain.com/api/simple-stream/configure           # 配置单个频道
https://yoyo-vps.your-domain.com/api/simple-stream/batch-configure    # 批量配置频道
https://yoyo-vps.your-domain.com/api/simple-stream/start-watching     # 开始观看频道
https://yoyo-vps.your-domain.com/api/simple-stream/stop-watching      # 停止观看频道
https://yoyo-vps.your-domain.com/api/simple-stream/heartbeat          # 会话心跳
https://yoyo-vps.your-domain.com/api/simple-stream/channel/:id/status # 频道状态
https://yoyo-vps.your-domain.com/api/simple-stream/system/status      # 系统状态
```
...
```
用户点击频道 (yoyo.your-domain.com)
    ↓
Cloudflare Workers API (yoyoapi.your-domain.com)
    ↓ 调用 start-watching
VPS SimpleStreamManager (yoyo-vps.your-domain.com)
    ↓ 检查是否首个观看者
如果是首个观看者 → 启动FFmpeg进程
    ↓
RTMP源实时读取 (0.5秒分片)
...
Nginx文件服务 (端口52535)
    ↓
Cloudflare Workers HLS代理 (yoyoapi.your-domain.com/hls/)
    ↓
前端hls.js播放器 (超低延迟播放)
```
...
# 测试API连通性
curl -X GET https://yoyo-vps.your-domain.com/health
curl -X GET https://yoyoapi.your-domain.com/api/streams
...
### 快速测试流程
1. 访问 `https://yoyo.your-domain.com`
...
```javascript
// 登录
const loginResponse = await fetch('https://yoyoapi.your-domain.com/api/login', {
  method: 'POST',
...
// 启动转码
const playResponse = await fetch('https://yoyoapi.your-domain.com/api/play/test-stream', {
  method: 'POST',
  credentials: 'include'
});

// 查询状态
const statusResponse = await fetch('https://yoyoapi.your-domain.com/api/stream/test-stream/status', {
  credentials: 'include'
});
```
...
```javascript
// 配置频道
const configResponse = await fetch('https://yoyo-vps.your-domain.com/api/simple-stream/configure', {
  method: 'POST',
  headers: { 
...
// 开始观看（自动启动转码）
const watchResponse = await fetch('https://yoyo-vps.your-domain.com/api/simple-stream/start-watching', {
  method: 'POST',
  headers: { 
    'Content-Type': 'application/json',
    'X-API-Key': '<VPS_API_KEY>'
...
// 查询系统状态
const systemStatus = await fetch('https://yoyo-vps.your-domain.com/api/simple-stream/system/status', {
  headers: { 
    'X-API-Key': '<VPS_API_KEY>'
  }
});
...
// 停止观看（自动清理）
const stopResponse = await fetch('https://yoyo-vps.your-domain.com/api/simple-stream/stop-watching', {
  method: 'POST',
  headers: { 
    'Content-Type': 'application/json',
    'X-API-Key': '<VPS_API_KEY>'
...
```
频道配置冲突:
- stream_ensxma2g 和 stream_kcwxuedx 共享: <RTMP_URL>
- stream_gkg5hknc 和 stream_kil0lecb 共享: <RTMP_URL>
...
// 修复后的真实RTMP地址映射
const realRtmpUrls = {
  'stream_ensxma2g': '<RTMP_URL>',
  'stream_gkg5hknc': '<RTMP_URL>',
  // ... 其他频道
};
```
...
- 频道名称在主页面正确更新显示
- 点击频道成功启动视频播放
- HLS URL正确生成：`https://yoyoapi.your-domain.com/hls/stream_cpa2czoo/playlist.m3u8?token=...`
- 视频状态显示："播放中"
...
#### 当前架构瓶颈
```
中国用户 → Cloudflare CDN → Workers → 美国VPS (yoyo-vps.your-domain.com)
```
...
#### 生产环境部署 ✅
- **前端应用**: https://yoyo.your-domain.com (Cloudflare Pages)
- **API服务**: https://yoyoapi.your-domain.com (Cloudflare Workers)
- **VPS转码**: https://yoyo-vps.your-domain.com (VPS + Nginx)
- **隧道优化**: tunnel-*.yoyo-vps.your-domain.com (Cloudflare Tunnel)
...
#### 生产环境验证
- **前端**: https://yoyo.your-domain.com ✅ 正常访问
- **API**: https://yoyoapi.your-domain.com ✅ 正常响应
- **VPS**: https://yoyo-vps.your-domain.com ✅ 正常运行
...
```powershell
# 1. 检查VPS整体状态
$status = Invoke-RestMethod -Uri "https://yoyo-vps.your-domain.com/api/deployment/status" -Method GET -TimeoutSec 10

# 2. 执行一键部署
$deployBody = @{ scriptName = "integrate-proxy-streaming.sh" } | ConvertTo-Json
$deployResult = Invoke-RestMethod -Uri "https://yoyo-vps.your-domain.com/api/deployment/deploy/complete" -Method POST -Body $deployBody -ContentType "application/json" -TimeoutSec 300

# 3. 验证部署结果
$healthCheck = Invoke-RestMethod -Uri "https://yoyo-vps.your-domain.com/health" -Method GET -TimeoutSec 5
$proxyStatus = Invoke-RestMethod -Uri "https://yoyo-vps.your-domain.com/api/proxy/status" -Method GET -TimeoutSec 10
```

##### 方案2: 安全的SSH连接设置
```bash
# 使用超时参数的SSH连接
ssh -o ConnectTimeout=10 -o ServerAliveInterval=5 root@<VPS_IP> "command"

# 避免使用的危险命令
ssh root@<VPS_IP> "git pull && pm2 restart app"  # ❌ 会卡死
ssh root@<VPS_IP> "nohup command &"             # ❌ 会卡死
```

##### 方案3: 分步骤验证
```powershell
# 1. 先测试连接
ssh -o ConnectTimeout=5 root@<VPS_IP> "echo 'connected'"

# 2. 再执行简单命令
ssh -o ConnectTimeout=10 root@<VPS_IP> "ls -la /opt/yoyo-transcoder"

# 3. 最后通过API验证结果
Invoke-RestMethod -Uri "https://yoyo-vps.your-domain.com/health"
```
...
### 🔧 紧急修复Git
```bash
ssh root@<VPS_IP> "rm -rf /tmp/github/secure-streaming-platform && mkdir -p /tmp/github && cd /tmp/github && git clone git@github.com:shao-ye/secure-streaming-platform.git"
```
